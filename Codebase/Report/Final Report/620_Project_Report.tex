...
The two different scalers that were used were \texttt{StandardScaler()} and  \texttt{RobustSclaer()}. The regressors were Linear, Lasso, Ridge, Elastic Net, and K Neighbors. Below is the code for the model implmentation:
...
preprocessor = ColumnTransformer(
    transformers=[
        ('num', RobustScaler(), numericalFeatures)
    ])
...
results.plot(kind='bar', y='Mean Squared Error', ax=ax[0], color='skyblue', title='Mean Squared Error by Model')
results.plot(kind='bar', y='R-squared', ax=ax[1], color='orange', title='R-squared by Model')
